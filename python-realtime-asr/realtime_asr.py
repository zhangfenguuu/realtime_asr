...
def send_audio(ws):
    ...
    '''
    duration = 5120  # 录制时长（单位：ms）
    sample_rate = 44100  # 采样率
    channels = 2  # 声道数

    print("开始录制...")
    recording = sd.rec(int(duration * sample_rate / 1000), samplerate=sample_rate, channels=channels)
    sd.wait()  # 等待录制完成

    print("保存为PCM文件...")
    audio = AudioSegment(data=recording[0].tobytes(), frame_rate=sample_rate, sample_width=2, channels=channels)
    audio.export('aa.pcm', format='pcm')
